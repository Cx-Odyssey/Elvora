...
function renderAccount(){
  ...
  el.innerHTML='<div class="acc-hero"><div class="acc-av">'+avHtml+'</div><div class="acc-name">'+currentUser.fname+' '+(currentUser.lname||'')+'</div><div class="acc-email">'+currentUser.email+'</div></div>'
    ...
    +'<div class="mitem" onclick="navPolicies()"><div class="mitem-ic"><svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg></div><div class="mitem-lbl">Policies</div><div class="mitem-arr">›</div></div>'
    +'<div class="mitem" onclick="navFAQ()"><div class="mitem-ic"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg></div><div class="mitem-lbl">FAQ</div><div class="mitem-arr">›</div></div>'
    +'<div class="mitem" onclick="navContact()"><div class="mitem-ic"><svg viewBox="0 0 24 24"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg></div><div class="mitem-lbl">Contact Us</div><div class="mitem-arr">›</div></div></div>'
...
function renderMyOrders(){
  ...
  var sc={Delivered:{bg:'rgba(82,200,154,.1)',c:'var(--green)'},Shipped:{bg:'rgba(120,80,220,.1)',c:'#a080e0'},Processing:{bg:'rgba(212,165,70,.1)',c:'var(--gold)'},Confirmed:{bg:'var(--surface)',c:'var(--muted)'},Cancelled:{bg:'rgba(224,92,58,.1)',c:'#e05c3a'}};
...
function showOrderDetail(oid){
  ...
  var cancelBtn=order.status==='Confirmed'?'<button onclick="cancelOrder(\''+order.id+'\')" style="width:100%;padding:13px;border-radius:12px;background:none;border:1.5px solid rgba(224,92,58,.3);color:#e05c3a;font-family:var(--sans);font-size:14px;font-weight:600;cursor:pointer;margin-top:10px;">Cancel Order</button>':'';
  var ac=document.getElementById('authContent');
...
function cancelOrder(oid){
  ...
  localStorage.setItem('el_orders',JSON.stringify(orders));
  if(currentUser&&order.user_id===currentUser.id)cancelOrderInBackend(oid);
...
function renderWishlist(){
  ...
  var h='<div style="font-family:var(--serif);font-size:22px;font-weight:400;color:var(--text);padding:4px 0 14px;"><span style="font-family:var(--sans);font-weight:600;">'+prods.length+'</span> Saved Item'+(prods.length>1?'s':'')+'</div><div class="pgrid" style="padding:0;">';
