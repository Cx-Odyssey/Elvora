...
function renderAccount(){
  ...
  el.innerHTML='<div class="acc-hero"><div class="acc-av"><svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div><div class="acc-name">'+currentUser.fname+' '+(currentUser.lname||'')+'</div><div class="acc-email">'+currentUser.email+'</div></div>'
    ...
    +'<div class="mitem" onclick="navOutletInfo()"><div class="mitem-ic"><svg viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg></div><div class="mitem-lbl">Our Store</div><div class="mitem-arr">›</div></div></div>'
    ...
    +'<div class="mitem" onclick="toastReturnPolicy()"><div class="mitem-ic"><svg viewBox="0 0 24 24"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 102.13-9.36L1 10"/></svg></div><div class="mitem-lbl">Returns &amp; Refunds</div><div class="mitem-arr">›</div></div></div>'
...
function renderWishlist(){
  ...
  prods.forEach(function(p){h+='<div class="pcard" onclick="openProd('+p.id+')"><div class="pcard-img" style="background:'+getGrad(p.category)+';"><div class="pcard-ill">'+getIll(p.category,false)+'<span>'+p.category+'</span></div>'+(p.badge?'<div class="pbadge best">'+p.badge+'</div>':'')+'<button class="pwish" onclick="event.stopPropagation();toggleWish('+p.id+')"><svg viewBox="0 0 24 24" style="width:13px;height:13px;stroke:var(--rose);stroke-width:2;fill:var(--rose);stroke-linecap:round;stroke-linejoin:round;"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg></button></div><div class="pinfo"><div class="pbrand">'+p.brand+'</div><div class="pname">'+p.name+'</div><div class="ppr-row"><span class="pprice">৳'+p.price.toLocaleString()+'</span><button class="padd" onclick="event.stopPropagation();addToCart('+p.id+',1)">+</button></div></div></div>';});
  el.innerHTML=h+'</div>';
}
...
function renderRecent(){
  ...
  list.innerHTML=prods.map(function(p){return '<div class="rvc" onclick="openProd('+p.id+')"><div class="rvc-img" style="background:'+getGrad(p.category)+';opacity:.85;">'+getIll(p.category,false).replace(/width="\d+"/,'style="width:30px;height:30px;opacity:.8;"')+'</div><div class="rvc-name">'+p.name+'</div><div class="rvc-price">৳'+p.price.toLocaleString()+'</div></div>';}).join('');
}
