...
function renderRecent(){
  ...
  list.innerHTML=prods.map(function(p){var fb=getIll(p.category,false).replace(/width="\d+"/,'style="width:30px;height:30px;opacity:.8;"');return '<div class="rvc" onclick="openProd('+p.id+')"><div class="rvc-img" style="background:'+getGrad(p.category)+';opacity:.85;">'+getProdVisual(p,fb)+'</div><div class="rvc-name">'+p.name+'</div><div class="rvc-price">৳'+p.price.toLocaleString()+'</div></div>';}).join('');
}
