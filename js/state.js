...
var currentUser=null; // populated by restoreSession() in auth.js on page load, if a Supabase session exists
...
var buyNowItem=JSON.parse(sessionStorage.getItem('el_buynow')||'null'); // set by pdBuy() for a single-product checkout that never touches the cart; persisted so an iOS Safari tab reload mid-checkout doesn't silently wipe it
var lastCheckoutItems=JSON.parse(sessionStorage.getItem('el_checkout_snapshot')||'null'); // exactly what renderCheckout() last showed the customer — placeOrder() trusts THIS, not a fresh re-derivation, so a total shown on screen can never silently become "nothing to order" if buyNowItem/cart get reset in between
...
/* Returns an <img> tag if the product has a real photo (image_url in
   Supabase), otherwise returns the fallback markup passed in (the
   gradient + icon illustration). Used everywhere a product visual
   is shown: product grid, detail page, cart, wishlist, recently viewed. */
function getProdVisual(p,fallbackHtml){
  if(p.image)return '<img src="'+p.image+'" alt="" loading="lazy" style="width:100%;height:100%;object-fit:cover;position:relative;z-index:2;"/>';
  return fallbackHtml;
}
