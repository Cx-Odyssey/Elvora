...
async function notifyMe(){
  var e=document.getElementById('notifyEmail')?document.getElementById('notifyEmail').value.trim():'';
  if(!e||!valEmail(e)){toast('Please enter a valid email');return;}
  ...
  if(supabaseClient){
    var context=document.getElementById('csTitle')?document.getElementById('csTitle').textContent:null;
    supabaseClient.from('notify_requests').insert([{email:e,context:context}]).then(function(res){
      if(res.error)console.warn('ELVORA: notify request failed to sync to Supabase',res.error);
    });
  }
  closeModal('csModal');toast('You will be first to know! ✦');
}

async function nlSub(){
  var e=document.getElementById('nlEmail')?document.getElementById('nlEmail').value.trim():'';
  if(!valEmail(e)){toast('Please enter a valid email');return;}
  var subs=JSON.parse(localStorage.getItem('el_subs')||'[]');
  if(subs.includes(e)){toast('Already subscribed!');return;}
  subs.push(e);localStorage.setItem('el_subs',JSON.stringify(subs));
  document.getElementById('nlEmail').value='';
  if(supabaseClient){
    supabaseClient.from('newsletter_subscribers').insert([{email:e}]).then(function(res){
      if(res.error&&res.error.code!=='23505')console.warn('ELVORA: newsletter signup failed to sync',res.error);
    });
  }
  toast('Subscribed! Welcome to the ELVORA family ✦');
}
