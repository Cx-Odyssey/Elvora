...
function addToCart(id,qty){
  ...
  if(ex)ex.qty+=qty;else cart.push({id:id,qty:qty,price:price,name:p.name,brand:p.brand,variant:label,category:p.category});
...
function renderCart(){
  ...
  el.innerHTML=cart.map(function(item,idx){
    var img=getIll(item.category||'serum',false).replace(/width="\d+"/,'style="width:34px;height:34px;"');
    return '<div class="citem"><div class="citem-img" style="background:'+getGrad(item.category||'serum')+';">'+img+'</div>'
