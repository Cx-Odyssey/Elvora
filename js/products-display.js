...
function renderProducts(list,tgt){
  ...
  grid.innerHTML=list.map(function(p,idx){
    ...
    var ev=getEventPrice(p);
    var old=ev.oldPrice?'<span class="pold">৳'+ev.oldPrice.toLocaleString()+'</span>':'';
    var badge=p.badge?'<div class="pbadge '+(p.badgeType||'best')+'">'+p.badge+'</div>':(ev.onEvent?'<div class="pbadge new2">'+activeEvent.discount_percent+'% OFF</div>':'');
    return '<div class="pcard" style="animation:fadeIn .4s ease '+(idx*60)+'ms both;" onclick="openProd('+p.id+')">'
      +'<div class="pcard-img" style="background:'+getGrad(p.category)+';">'+getProdVisual(p,'<div class="pcard-ill">'+getIll(p.category,false)+'<span>'+p.category+'</span></div>')
      ...
      +'<div class="pstars">'+stars+'</div>'
      +'<div class="ppr-row"><div><span class="pprice">৳'+ev.price.toLocaleString()+'</span>'+old+'</div>'
...
function filterCat(cat,btn){
  ...
  document.querySelectorAll('.concern-chip').forEach(function(c){c.classList.remove('sel');});var allConcern=document.querySelector('.concern-chip');if(allConcern)allConcern.classList.add('sel');
  renderProducts(cat==='all'?PRODUCTS:PRODUCTS.filter(function(p){return p.category===cat;}));
}

function filterConcern(concern,btn){
  document.querySelectorAll('.concern-chip').forEach(function(c){c.classList.remove('sel');});if(btn)btn.classList.add('sel');
  document.querySelectorAll('.cat').forEach(function(c){c.classList.remove('sel');});var allCat=document.querySelector('.cat');if(allCat)allCat.classList.add('sel');
  renderProducts(concern==='all'?PRODUCTS:PRODUCTS.filter(function(p){return p.concerns&&p.concerns.includes(concern);}));
}
...
function openProd(id){
  ...
  var ph=document.getElementById('pdImgPh');
  var hint=pdImg?pdImg.querySelector('.pd-img-hint'):null;
  var realImg=pdImg?pdImg.querySelector('img.pd-real-img'):null;
  if(p.image){
    if(ph)ph.style.display='none';
    if(hint)hint.style.display='none';
    if(!realImg){
      realImg=document.createElement('img');
      realImg.className='pd-real-img';
      realImg.loading='lazy';
      realImg.style.cssText='width:100%;height:100%;object-fit:cover;position:absolute;inset:0;z-index:1;';
      if(pdImg)pdImg.appendChild(realImg);
    }
    realImg.src=p.image;realImg.alt=p.name;
  }else{
    if(ph)ph.style.display='flex';
    if(hint)hint.style.display='block';
    if(realImg)realImg.remove();
  }
  ...
  s=document.getElementById('pdRating');if(s)s.textContent=p.reviews?(p.rating+' · '+p.reviews+' reviews'):'';
  var ev=getEventPrice(p);
  var dp=applyEventDiscount(selVarP||p.price);
  s=document.getElementById('pdPrice');if(s)s.textContent='৳'+dp.toLocaleString();
  s=document.getElementById('pdOld');if(s)s.textContent=ev.oldPrice?'৳'+ev.oldPrice.toLocaleString():'';
  var de=document.getElementById('pdDisc');if(de){if(ev.onEvent){de.textContent=activeEvent.discount_percent+'% off';de.style.display='block';}else if(p.oldPrice){de.textContent=Math.round((1-p.price/p.oldPrice)*100)+'% off';de.style.display='block';}else de.style.display='none';}
  ...
  if(vs){if(p.variants&&p.variants.length){var vh='<div class="pd-lbl">Size — Select</div><div class="vrow">';p.variants.forEach(function(v,i){var vp=applyEventDiscount(v.price);vh+='<div class="vchip'+(i===0?' sel':'')+'" onclick="selVar(this,'+v.price+',\''+v.label+'\')">'  +v.label+' — ৳'+vp.toLocaleString()+'</div>';});vh+='</div>';vs.innerHTML=vh;selVarP=p.variants[0].price;selVarL=p.variants[0].label;document.getElementById('pdPrice').textContent='৳'+applyEventDiscount(p.variants[0].price).toLocaleString();}else vs.innerHTML='';}
...
async function renderPdRevs(id,p){
  var re=document.getElementById('pdReviews');if(!re)return;
  re.innerHTML='<div style="text-align:center;padding:28px 0;font-size:13px;color:var(--muted);">Loading reviews...</div>';
  var all=[];
  if(supabaseClient){
    try{
      var res=await supabaseClient.from('reviews').select('*').eq('product_id',id).order('created_at',{ascending:false});
      if(res.error)throw res.error;
      all=(res.data||[]).map(function(r){return{name:r.author_name,rating:r.rating,text:r.body,verified:r.verified,avatar:r.avatar_url,date:new Date(r.created_at).toLocaleDateString('en-GB',{month:'short',year:'numeric'})};});
    }catch(e){console.warn('ELVORA: could not load reviews',e);}
  }
  if(curProd!==id)return; // user navigated away while this was loading
  ...
  re.innerHTML=all.map(function(r){var av=r.avatar?'<img src="'+r.avatar+'" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;"/>':'🌸';return '<div class="rv-item"><div class="rv-head"><div class="rv-info"><div class="rv-av">'+av+'</div><div><div class="rv-name">'+r.name+'</div><div class="rv-date">'+r.date+'</div></div></div><div class="rv-stars"><span>'+('★'.repeat(r.rating))+'</span></div></div><div class="rv-text">'+r.text+'</div>'+(r.verified?'<div class="verified">✓ Verified Purchase</div>':'')+'</div>';}).join('');
}

function chQty(d){pdQtyN=Math.max(1,pdQtyN+d);var el=document.getElementById('pdQtyN');if(el)el.textContent=pdQtyN;}

function selVar(el,price,label){document.querySelectorAll('.vchip').forEach(function(c){c.classList.remove('sel');});el.classList.add('sel');selVarP=price;selVarL=label;var pe=document.getElementById('pdPrice');if(pe)pe.textContent='৳'+applyEventDiscount(price).toLocaleString();}

function pdAdd(){if(!curProd)return;addToCart(curProd,pdQtyN);}

function pdBuy(){
  if(!curProd)return;
  var p=PRODUCTS.find(function(x){return x.id===curProd;});if(!p)return;
  buyNowItem={id:p.id,qty:pdQtyN,price:applyEventDiscount(selVarP||p.price),name:p.name,brand:p.brand,variant:selVarL||null,category:p.category,image:p.image||null};
  sessionStorage.setItem('el_buynow',JSON.stringify(buyNowItem));
  nav('checkout');renderCheckout();
}
