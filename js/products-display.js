...
async function renderPdRevs(id,p){
  ...
  if(supabaseClient){
    try{
      ...
      all=(res.data||[]).map(function(r){return{name:r.author_name,rating:r.rating,text:r.body,verified:r.verified,date:new Date(r.created_at).toLocaleDateString('en-GB',{month:'short',year:'numeric'})};});
