/* ═══════════════════════════════════════════════════════════
   ...
   IMPORTANT: renderCheckout() snapshots exactly what it shows
   into lastCheckoutItems (persisted to sessionStorage too) the
   moment it renders. placeOrder() trusts THAT snapshot rather
   than re-deriving from buyNowItem/cart fresh — so if anything
   resets those in the background between page-render and the
   customer tapping Place Order (an iOS tab reload, back/forward
   cache restoring a stale page, etc), the order they actually
   saw on screen still goes through instead of silently failing
   with "Nothing to order".
   ═══════════════════════════════════════════════════════════ */

function checkoutItems(){return buyNowItem?[buyNowItem]:cart;}

function checkoutSubtotal(items){items=items||checkoutItems();return items.reduce(function(a,b){return a+b.price*b.qty;},0);}

function renderCheckout(){
  var items=checkoutItems();
  lastCheckoutItems=items;
  sessionStorage.setItem('el_checkout_snapshot',JSON.stringify(items));
  var sub=checkoutSubtotal(items);
  ...
  var itemsHtml=items.map(function(i){return '<div class="mrow"><span>'+i.name+(i.variant?' · '+i.variant:'')+' ×'+i.qty+'</span><span>৳'+(i.price*i.qty).toLocaleString()+'</span></div>';}).join('');
  ...
  el.innerHTML='<div class="back-row" onclick="goBack()"><div class="back-ic"><svg viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg></div><div class="back-lbl">Checkout</div></div>'
    +'<div class="omini">'+itemsHtml+promoRow+'<div class="mrow"><span>Delivery</span><span id="miniDel">'+(deliveryFee===0?'FREE':'৳'+deliveryFee)+'</span></div><div class="mrow tot"><span>Total</span><span id="miniTot">৳'+grandTotal.toLocaleString()+'</span></div></div>'
...
function placeOrder(){
  ...
  // Trust the snapshot taken when this checkout page rendered, not a fresh
  // re-derivation — this is what actually fixes "shows a total, then says
  // nothing to order" if buyNowItem/cart got reset by anything in between.
  var items=(lastCheckoutItems&&lastCheckoutItems.length)?lastCheckoutItems:checkoutItems();
  ...
  var order={id:oid,name:name,phone:phone,addr:addr,dist:dist,items:JSON.parse(JSON.stringify(items)),total:total,fee:deliveryFee,promo:appliedPromo?appliedPromo.code:null,user_id:currentUser?currentUser.id:null,date:new Date().toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'}),status:'Confirmed'};
  ...
  recordPromoUsage();
  if(buyNowItem){buyNowItem=null;sessionStorage.removeItem('el_buynow');}else{cart=[];saveCart();}
  lastCheckoutItems=null;sessionStorage.removeItem('el_checkout_snapshot');
  appliedPromo=null;updateBadges();fireConfetti();
