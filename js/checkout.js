...
function renderCheckout(){
  var sub=getTotal();if(sub>=1500)deliveryFee=0;
  ...
  el.innerHTML='<div class="back-row" onclick="goBack()"><div class="back-ic"><svg viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg></div><div class="back-lbl">Checkout</div></div>'
    +'<div class="omini">'+items+'<div class="mrow"><span>Delivery</span><span id="miniDel">'+(deliveryFee===0?'FREE':'৳'+deliveryFee)+'</span></div><div class="mrow tot"><span>Total</span><span id="miniTot">৳'+(sub+deliveryFee).toLocaleString()+'</span></div></div>'
    ...
    +'<button class="placebtn" id="placeBtn" onclick="placeOrder()">Place Order — Cash on Delivery (৳'+(sub+deliveryFee).toLocaleString()+')</button>'
...
function placeOrder(){
  ...
  var sub=getTotal();var total=sub+deliveryFee;
  var oid='EL-'+Date.now().toString().slice(-7);
  var order={id:oid,name:name,phone:phone,addr:addr,dist:dist,items:JSON.parse(JSON.stringify(cart)),total:total,fee:deliveryFee,date:new Date().toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'}),status:'Confirmed'};
  ...
  cart=[];saveCart();updateBadges();fireConfetti();
...
function fireConfetti(){
  var cols=['#d4a566','#e8c898','#c45a78'];
  for(var i=0;i<16;i++){
    var c=document.createElement('div');
    var sz=3+Math.random()*4;
    c.style.cssText='position:fixed;width:'+sz+'px;height:'+sz+'px;background:'+cols[Math.floor(Math.random()*cols.length)]+';border-radius:50%;opacity:.85;left:'+Math.random()*100+'vw;top:-10px;z-index:9999;animation:confettiFall '+(2+Math.random()*1.5)+'s ease forwards;animation-delay:'+Math.random()*.5+'s;pointer-events:none;';
